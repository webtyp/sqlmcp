...
# Plan — `webtyp.com/sqlmcp`: a list stops claiming it has columns
...
**Files:** `models_orm.go` (2 list types).
...
3. Confirm the diff contains **only** removals of the two stub methods —
   2 `Schema()` and 2 `Pointers()` lines — and nothing else. If
...
| Stage | Files | Action |
|---|---|---|
| 1 | `models_orm.go` | regenerate with `ormc`; 2 stub pairs disappear |
